...
MODEL_PREFILL_SOURCE_PATHS = (
    Path(__file__),
    ROOT / "diagnostics/aggregate_pg19_external_quality.py",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--token-source",
        choices=("wikitext2", "pg19", "random"),
        default="wikitext2",
        help=(
            "wikitext2 reads and locally tokenizes the selected raw member; "
            "pg19 reads one manifest-locked official test book; "
    ...
    parser.add_argument(
        "--pg19-file",
        type=Path,
        help="local official PG-19 test-book text file",
    )
    parser.add_argument(
        "--pg19-object-name",
        help="manifest-locked GCS object name, for example test/10146.txt",
    )
    parser.add_argument(
        "--pg19-selection-manifest",
        type=Path,
        help="pre-content-access PG-19 selection and configuration manifest",
    )
    parser.add_argument(
        "--wikitext-zip",
        type=Path,
...
def request_window_metadata(
    ...
    for request in range(batch_size):
        if starts is None:
            ...
            source_kind = str(token_provenance.get("kind"))
            unit_id = (
                f"{source_kind}-{split}-{member_sha256[:16]}-"
                f"{label_begin}-{label_end}"
            )
        records.append(
            {
                "request": request,
...
def build_token_matrix(
    ...
    Corpus text is read from a hash-attested local artifact.  Each request is
    ``BOS + a disjoint corpus window``; therefore the first decode token is the
    real continuation of its prefix, and position ``context + decode_steps``
    labels the final decode logit.
    """
    ...
    if args.token_source == "random":
        ...
        if args.token_source == "wikitext2":
        ...
        elif args.token_source == "pg19":
            if args.batch_size != 1:
                raise ValueError("PG-19 external validation requires batch size one")
            if args.pg19_file is None or not args.pg19_file.is_file():
                raise ValueError(f"PG-19 text file not found: {args.pg19_file}")
            if not args.pg19_object_name:
                raise ValueError("PG-19 requires --pg19-object-name")
            manifest_path = args.pg19_selection_manifest
            if manifest_path is None or not manifest_path.is_file():
                raise ValueError(
                    f"PG-19 selection manifest not found: {manifest_path}"
                )
            manifest_bytes = manifest_path.read_bytes()
            manifest = json.loads(manifest_bytes.decode("utf-8"))
            if manifest.get("selection_frozen_before_corpus_content_access") is not True:
                raise ValueError("PG-19 selection was not frozen before content access")
            expected_configuration = manifest.get("configuration")
            observed_configuration = {
                ...
                "minimum_flashinfer_hf_cosine": args.min_baseline_hf_cosine,
                "disable_attention_cuda_graphs": args.disable_attention_cuda_graphs,
            }
            if expected_configuration != observed_configuration:
                raise ValueError(
                    "PG-19 invocation differs from the frozen configuration: "
                    f"observed={observed_configuration!r}, "
                    f"expected={expected_configuration!r}"
                )
            matches = [
                record
                for record in manifest.get("selected_objects", [])
                if record.get("name") == args.pg19_object_name
            ]
            if len(matches) != 1:
                raise ValueError(
                    "PG-19 object is not uniquely selected by the frozen manifest"
                )
            object_record = matches[0]
            member_bytes = args.pg19_file.read_bytes()
            if len(member_bytes) != int(object_record["size_bytes"]):
                raise ValueError("PG-19 object byte length differs from manifest")
            observed_md5 = base64.b64encode(
                hashlib.md5(member_bytes, usedforsecurity=False).digest()
            ).decode("ascii")
            if observed_md5 != object_record["gcs_md5_base64"]:
                raise ValueError("PG-19 object MD5 differs from canonical GCS metadata")
            member_sha256 = sha256_bytes(member_bytes)
            text = member_bytes.decode("utf-8")
            source_specific = {
                "kind": "pg19",
                "dataset": "PG-19 Language Modelling Benchmark",
                "split": "test",
                "official_homepage": "https://github.com/google-deepmind/pg19",
                "official_bucket": "gs://deepmind-gutenberg",
                "object_name": args.pg19_object_name,
                "object_generation": object_record["generation"],
                "object_size_bytes": len(member_bytes),
                "object_gcs_md5_base64": object_record["gcs_md5_base64"],
                "object_gcs_md5_verified": True,
                "document_path": str(args.pg19_file.resolve()),
                "document_sha256": member_sha256,
                "selection_manifest_path": str(manifest_path.resolve()),
                "selection_manifest_sha256": sha256_bytes(manifest_bytes),
                "selection_rule": manifest["selection_rule"],
                # Compatibility aliases used by the unchanged metric serializer.
                "archive_member": args.pg19_object_name,
                "archive_member_sha256": member_sha256,
                "archive_member_uncompressed_bytes": len(member_bytes),
            }
        else:
            raise AssertionError(f"unsupported token source {args.token_source!r}")
        ...
        if stride < corpus_tokens_per_request:
            raise ValueError(
                "corpus request windows must be disjoint: token stride must "
        ...
        if starts[-1] + corpus_tokens_per_request > len(encoded):
            raise ValueError(
                "the selected corpus document does not contain enough token IDs for "
        ...
        provenance = {
            **source_specific,
            ...
            "continuation_semantics": (
                ...
                "all later decode IDs are actual corpus tokens; each decode "
...
def main() -> None:
    ...
    prefix_oracle = {
        "definition": (
            "HF SDPA FP16 final-prefill logits predict the actual corpus "
