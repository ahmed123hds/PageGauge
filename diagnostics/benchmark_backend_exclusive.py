...
def cache_tensor_manifest(cache: Any, backend: str) -> dict[str, Any]:
    ...
    total = 0
    if backend == "page_gauge" and hasattr(cache, "value_channel_gain"):
        attributes = (*attributes, "value_channel_gain")
...
def populate_backend_cache_layer(
    *,
    ...
    # Optional setup-only reparameterization. HF source/oracle tensors stay intact.
    source_value = PG.VALUE_CONDITIONING.condition_prefill(cache, layer, request, source_value)
