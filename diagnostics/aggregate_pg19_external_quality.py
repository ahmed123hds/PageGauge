#!/usr/bin/env python3
"""Reduce six preregistered PG-19 books under frozen S4/A128/T768."""
...
def generic_cluster_bootstrap(
    ...
    require(
        len(clusters) == len(OBJECT_NAMES),
        "bootstrap requires exactly six book clusters",
    )
...
def validate_record(
    path: Path, expected_object: str, manifest_sha256: str
    ...
    require(token_source.get("kind") == "pg19", f"{label}: source kind")
    require(
        token_source.get("dataset") == "PG-19 Language Modelling Benchmark",
        f"{label}: dataset",
    )
    require(token_source.get("split") == "test", f"{label}: split")
    require(
        token_source.get("object_name") == expected_object,
        f"{label}: object",
    )
    require(
        token_source.get("selection_manifest_sha256") == manifest_sha256,
        f"{label}: selection manifest",
    )
    require(
        token_source.get("corpus_window_start_offsets") == [0],
        f"{label}: start",
    )
...
def main() -> None:
    ...
    require(
        selection.get("selection_frozen_before_corpus_content_access") is True,
        "selection manifest was not frozen before content access",
    )
    require(
        tuple(record["name"] for record in selection.get("selected_objects", []))
        == OBJECT_NAMES,
        "selection manifest object list differs from frozen six-book cohort",
    )
    manifest_sha256 = sha256_file(args.selection_manifest)
    keyed: dict[str, Path] = {}
    for path in args.inputs:
        payload = json.loads(path.read_text(encoding="utf-8"))
        object_name = payload.get("token_source", {}).get("object_name")
        require(isinstance(object_name, str), f"{path}: object name required")
        require(object_name not in keyed, f"duplicate object {object_name}")
        keyed[object_name] = path
    require(
        tuple(name for name in OBJECT_NAMES if name in keyed) == OBJECT_NAMES
        and len(keyed) == len(OBJECT_NAMES),
        "inputs do not match the six fixed PG-19 books",
    )
    ...
    for object_name in OBJECT_NAMES:
        record, windows = validate_record(
            keyed[object_name], object_name, manifest_sha256
        )
        ...
        require(pg_labels == hf_labels, f"{keyed[object_name]}: label mismatch")
        ...
        input_manifest.append(
            {
                "path": str(keyed[object_name]),
                "sha256": record["sha256"],
                "pg19_object_name": object_name,
                "document_sha256": record["payload"]["token_source"][
                    "document_sha256"
                ],
    ...
    result = {
        "schema_version": 1,
        "experiment": "page_gauge_s4_a128_t768_pg19_external_confirmation",
        ...
        "protocol": {
            ...
            "pg19_test_objects": list(OBJECT_NAMES),
            "cluster_unit": "one independent PG-19 test book",
            "cluster_count": len(OBJECT_NAMES),
            "total_labeled_tokens": len(OBJECT_NAMES) * DECODE_STEPS,
            "bootstrap_samples": BOOTSTRAP_SAMPLES,
            "selection_manifest_path": str(args.selection_manifest),
            "selection_manifest_sha256": manifest_sha256,
            "external_validation_disclosure": (
                "PG-19 was not used during PageGauge development or policy selection. "
                "The six-book cohort and S4/A128/T768 configuration were frozen before "
                "book content or metrics were accessed. This does not establish that the "
                "base Mistral model never encountered Project Gutenberg during pretraining."
            ),
        },
