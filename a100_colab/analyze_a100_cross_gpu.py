#!/usr/bin/env python3
"""Fail-closed reduction of the A100 S4/A128/T768 cross-GPU run."""
...
def validate_worker(path: Path, backend: str, seed: int, offset: int) -> dict[str, Any]:
    ...
    if backend == "page_gauge":
        ...
        require(
            hashes.get("header_sha256") == EXPECTED_HEADER_SHA256
            and hashes.get("header_matches_expected") is True,
            f"{path.name}: PageGauge vendor header",
        )
...
def main() -> None:
    ...
    result = {
        "schema_version": 1,
        "experiment": "page_gauge_a100_cross_gpu_s4_a128_t768",
