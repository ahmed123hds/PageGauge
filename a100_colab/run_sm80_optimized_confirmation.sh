...
EXPECTED_SELECTION_SHA256=914566c0704735d5d4dddb6df780d377785ecc306ee7f66cfe34532755bd680e
if [[ ! -s "$SELECTION" ]]; then
  echo "Missing frozen SM80 selection: $SELECTION" >&2
  exit 2
fi
observed_selection_sha256=$(sha256sum "$SELECTION" | awk '{print $1}')
if [[ "$observed_selection_sha256" != "$EXPECTED_SELECTION_SHA256" ]]; then
  echo "SM80 selection hash mismatch: $observed_selection_sha256" >&2
  exit 2
fi

OUTPUT_DIR=results/a100_sm80_optimized_s4_a128_t768/fresh_process_williams
...
for index in 0 1 2 3 4 5 6 7; do
  ...
  set +e
  command=(python diagnostics/benchmark_sustained_dynamic_graphs.py)
  if [[ "$backend" == page_gauge ]]; then
    command=(python a100_colab/run_with_sm80_kernel.py \
      --target sustained --cap 2 --exact-split-pages 30 --)
  fi
  "${command[@]}" \
    --backend "$backend" \
...
python a100_colab/analyze_sm80_optimized_confirmation.py \
  --run-dir "$OUTPUT_DIR" \
  ...
  --selection "$SELECTION" \
  --output results/a100_sm80_optimized_s4_a128_t768/final_a100_analysis.json
