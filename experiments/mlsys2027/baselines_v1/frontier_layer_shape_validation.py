...
def main():
    ...
    for source in args.fixtures:
        ...
        with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
            ...
            out = ROOT/'results/mlsys2027_baselines_v1'/('layer_shape_validation_'+m['backend']+'_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
            out.mkdir()
            ...
            for module in (segments, segment_graph_dispatch, mistral_dense_segments, attention, common):
            ...
            protocol = Path(__file__).with_name('LAYER_SHAPE_EXTENSION.md')
            m['source_sha256'][str(protocol.resolve())] = base.sha256_file(protocol)
