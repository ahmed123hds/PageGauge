...
def worker(out):
    ...
    from reusable_lowbit_generation import generate
    ...
    second = generate(model, contract, 'kivi', 4, eos)
    if result['generated_ids'] != second['generated_ids'] or type(model).__name__ != 'MistralForCausalLM':
        raise ValueError('Repeated request reuse changed output or failed HF restoration')
    result['repeated_request_identical'] = True
...
def main():
    ...
    with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
        ...
        out = ROOT/'results/mlsys2027_tasks_v1'/('reusable_lowbit_smoke_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
        out.mkdir()
        fixtures = json.loads((source/'fixtures.json').read_text())
        base.atomic_json(out/'fixtures.json', {'cases': fixtures['cases'][:1]})
        paths = [Path(__file__), Path(__file__).with_name('reusable_lowbit_generation.py'), Path(__file__).with_name('lowbit_generation.py'),
