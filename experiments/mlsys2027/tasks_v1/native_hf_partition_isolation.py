...
def worker(out):
    ...
    with torch.inference_mode():
        aligned_past = None
        for start in range(0, short.prefill_tokens, 256):
            output = model(ids[:, start:min(start+256, short.prefill_tokens)], past_key_values=aligned_past, use_cache=True)
            aligned_past = output.past_key_values
        # Native generate otherwise ingests all 16 remaining prompt tokens at once.
        # Match the adapter's pre-generation schedule through the penultimate token.
        for position in range(short.prefill_tokens, len(short.prompt_ids)-1):
            output = model(ids[:, position:position+1], past_key_values=aligned_past, use_cache=True)
            aligned_past = output.past_key_values
        matched = model.generate(ids, attention_mask=torch.ones_like(ids), past_key_values=aligned_past,
            do_sample=False, max_new_tokens=short.max_new_tokens, eos_token_id=eos,
            pad_token_id=eos[0], use_cache=True)[0, len(short.prompt_ids):].tolist()
    ...
    base.atomic_json(out/'diagnosis.json', {
        ...
        'matched_prefill_native_ids': matched, 'matched_prefill_equal': matched == manual['generated_ids'],
    ...
    if manual['generated_ids'] != matched:
        raise ValueError('Matched-prefill native generate still differs from manual rollout')
    result['matched_prefill_native_generate_equal'] = True
    result['full_prefill_native_generate_equal'] = manual['generated_ids'] == native
    verify(m)
...
def main():
    ...
    with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
        ...
        out = ROOT/'results/mlsys2027_tasks_v1'/('native_hf_partition_isolation_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
        out.mkdir()
