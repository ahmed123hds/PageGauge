...
def main():
    ...
    specs = [('nsn', 'nsn2_nsn_generation_20260910T093744Z_66c08f3e', 'nsn_int2'),
             ('kitty', 'kitty2_kitty_generation_20260910T093848Z_18cb321f', 'kitty_pro')]
    for family, source_name, quantized in specs:
        source = ROOT/'results/mlsys2027_tasks_v1'/source_name
        for backend in ('hf', quantized):
            ...
            with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
                ...
                out = ROOT/'results/mlsys2027_tasks_v1'/(family+'_'+backend+'_cohort_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
                out.mkdir()
                base.atomic_json(out/'fixtures.json', fixtures)
                worker = Path(__file__).with_name('native_full_task_worker.py')
