"""Long-prompt generated-task worker; requires externally frozen manifest.

No built-in data loading or answer scoring. Short prompts use explicitly disclosed independent native FP16 fallback.
"""
import argparse
from packed_hf_views import hf_projection_views
from task_contract import prepare, validate_result
from short_prompt_fallback import run as run_fallback
...
def worker(out):
    ...
    for index, case in enumerate(cases):
        ...
        prompt = list(contract.prompt_ids)
        m['max_new_tokens'] = contract.max_new_tokens
        context = contract.prefill_tokens
        maximum = len(prompt)+contract.max_new_tokens
        if contract.fp16_fallback:
            def native(prompt_ids, eos_ids, limit):
                with hf_projection_views(model):
                    return hf_greedy(model, prompt_ids, eos_ids, limit)
            arms = run_fallback(contract, eos, native)
            for arm in arms.values():
                arm['generated_text'] = tokenizer.decode(arm['generated_ids'], skip_special_tokens=True)
            result = {'case_id': case['case_id'], 'arms': arms,
                      'prompt_contract': {'task': contract.task, 'original_tokens': contract.original_prompt_tokens,
                       'served_tokens': len(prompt), 'removed_tokens': contract.removed_tokens,
                       'max_new_tokens': contract.max_new_tokens, 'fp16_fallback': True}}
            results.append(result)
            base.atomic_json(out/f'case_{index}.json', result)
            print('Explicit native fallback completed '+case['case_id'], flush=True)
            continue
        ...
        if index == 0:
            with hf_projection_views(model):
                official = model.generate(torch.tensor([prompt], device='cuda'), attention_mask=torch.ones((1, len(prompt)), device='cuda', dtype=torch.long), do_sample=False,
        ...
        result['prompt_contract'] = {'task': contract.task, 'original_tokens': contract.original_prompt_tokens,
            'served_tokens': len(prompt), 'removed_tokens': contract.removed_tokens,
            'max_new_tokens': contract.max_new_tokens, 'fp16_fallback': False}
        result['arms']['hf'] = hf
        ...
        for name, arm in result['arms'].items():
            arm['generated_text'] = tokenizer.decode(arm['generated_ids'], skip_special_tokens=True)
            validate_result(contract, arm['generated_ids'], arm['stop_reason'], eos)
        ...
        print('Generated all arms for '+case['case_id'], flush=True)
        del baseline, cos, sin; gc.collect(); torch.cuda.empty_cache()
    verify(m)
    base.atomic_json(out/'analysis.json', {'cases': len(results), 'results': results,
        ...
        'scope': m['scope'], 'model_loads': 1, 'execution_complete': True})
