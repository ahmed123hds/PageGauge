...
import sys
import uuid
import long_prompt_worker as synthetic
...
def main():
    ...
    with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
        ...
        out = ROOT/'results/mlsys2027_tasks_v1'/('task_contract_smoke_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
        out.mkdir()
        ...
        for path in (Path(__file__).resolve(), downloads[0], Path(synthetic.__file__).resolve(), Path(__file__).with_name('packed_hf_views.py'), Path(__file__).with_name('task_contract.py')):
