...
def worker(out):
    ...
    base.atomic_json(out/'analysis.json',{'passed':True,'prompt_tokens':len(prompt),
        ...
        'scope': 'One already-exposed synthetic Mistral-Instruct prompt, native HF before/after packed zero-copy projection views. Preparation optimization only; no new task score, timing claim, quantizer or decoder-kernel change.'})
...
def main():
    ...
    if args.worker:
        ...
        return
    import fcntl
    source = ROOT/'results/mlsys2027_tasks_v1/instruct_synthetic_20260910T050624Z_cb784d3c'
    m = json.loads((source/'manifest.json').read_text()); verify(m)
    for name in ('validate_instruct_hf_views.py','packed_hf_views.py'):
    ...
    with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
        ...
        out = ROOT/'results/mlsys2027_tasks_v1'/('instruct_packed_hf_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
