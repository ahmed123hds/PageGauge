...
def worker(out):
    ...
    model, make_cache, account = load(m['model'], 'kitty', m['task_backend'], 32768)
    ...
    base.atomic_json(out/'analysis.json', {'execution_complete': True, 'backend': m['task_backend'],
        'case_id': case['case_id'], 'result': result, 'scope': 'Native Kitty-stack synthetic own-generation only, not public quality.'})
...
def main():
    ...
    for backend, bits in (('hf', 0), ('kitty', 2)):
        ...
        with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
            ...
            out = ROOT/'results/mlsys2027_tasks_v1'/(backend+str(bits)+'_kitty_generation_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
            out.mkdir()
            fixtures = json.loads((source/'fixtures.json').read_text())
            fixtures['cases'] = fixtures['cases'][:1]
            fixtures['cases'][0]['task'] = 'qasper'
            ...
            paths.append(ROOT/'experiments/mlsys2027/baselines_v1/kitty_quality.py')
            paths.extend(Path('/home/anonymous/pagegauge_baselines/Kitty/src').rglob('*.py'))
            ...
            m.update(baseline=backend, bits=bits, idle=idle, task_backend='hf' if backend == 'hf' else 'kitty_pro',
                     fixtures_sha256=base.sha256_file(out/'fixtures.json'))
            base.atomic_json(out/'manifest.json', m)
            command = ['/home/anonymous/pagegauge_baselines/kitty_sm120_env/bin/python', '-u', str(Path(__file__)), '--worker', str(out)]
