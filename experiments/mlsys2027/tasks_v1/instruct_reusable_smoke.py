...
import sys
import uuid
import reusable_synthetic as synthetic
...
def main():
    ...
    cases = [synthetic.make_case(tokenizer, 8192, depth, 2026091016+i, 'mistral') for i, depth in enumerate((.5, .9))]
    idle = base.idle_preflight(0); gpu = idle[-1]['uuid']
    with (Path('/tmp')/f'pagegauge-mlsys-{gpu}.lock').open('a') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        out = ROOT/'results/mlsys2027_tasks_v1'/('instruct_reusable_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid.uuid4().hex[:8])
        out.mkdir()
        ...
        m.update(model=str(model), model_family='mistral', input_file_evidence=evidence,
            ...
            scope='Two synthetic Mistral-Instruct prompts with one model load; independent HF/FI/PG generation with first-case native HF-generate comparison. Reference policy S4/A128/T768. Not public-task score or optimized layer-runtime performance.')
        for path in (Path(__file__).resolve(), downloads[0], Path(synthetic.__file__).resolve(), Path(__file__).with_name('packed_hf_views.py')):
